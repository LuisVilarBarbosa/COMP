...
public class Parser {
    ...
    private ArrayList<AutoNode> syntacticAnalysisTrees;
    ...
    public ArrayList<AutoNode> getSyntacticAnalysisTrees() {
    ...
    private ArrayList<AutoNode> generateSyntacticAnalysisTrees(ArrayList<String> codeLines, ArrayList<PragmaScope> pragmaScopes) throws Exception {
        ArrayList<AutoNode> pragmaTrees = new ArrayList<>();
        for (int i = 0; i < pragmaScopes.size(); i++) {
            PragmaScope pragmaScope = pragmaScopes.get(i);
            AutoNode root = new AutoNode("");
    ...
    private void buildTree(String pragma, AutoNode root) throws Exception {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (numTabs == indentation) {
                AutoNode parent = root.getParent(lastNewAutoNode);
                if (parent == null)
                    parent = root;
                lastNewAutoNode = addNodeToTree(info, parent);
            } else if (numTabs == indentation - 1) {
                AutoNode parent = root.getParent(lastNewAutoNode);
                AutoNode grandparent = root.getParent(parent);
                lastNewAutoNode = addNodeToTree(info, grandparent);
            } else if (numTabs == indentation + 1) {
                lastNewAutoNode = addNodeToTree(info, lastNewAutoNode);
            } else
    ...
    private AutoNode addNodeToTree(String info, AutoNode parent) {
        AutoNode n = new AutoNode(info);
        parent.addChild(n);
        return n;
    }
}
